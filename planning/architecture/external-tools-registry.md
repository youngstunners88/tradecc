...
|---|---|---|
| OpenBB | github.com/OpenBB-finance/OpenBB | **AGPLv3** (73.2k stars) — the same licence clause reviewed for PineTS on 2026-09-14; §13 network-provision is the forward risk. Touches *data access*, one of the two real binding constraints, but on the **breadth** axis, not the **point-in-time-cleanliness** axis that actually blocks copy-trading and pump.fun. Its docs say nothing about PIT snapshots, as-reported-vs-restated, or survivorship, and it disclaims that its data "is not necessarily accurate". **Evaluate only the narrow question: does any OpenBB provider expose as-reported-at-the-time history?** Do not adopt on breadth. See `2026-09-19-ai-trading-repo-review.md` |
