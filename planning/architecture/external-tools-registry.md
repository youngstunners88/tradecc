...
| `astra-analyst` skill | (built in-house) | **Reviewed and committed DORMANT (2026-09-13).** Threat review C1/C2/M4/L2 actioned: no side-effecting tools, prompts built from a field allowlist rather than "runtime state minus redactions", the caller-side verdict rule stated, and config/risk/gate declared off-limits to model output. **Not wired up** — every use it lists has lost its subject (copy-trading blocked, regime refuted, nothing running), so activating it would buy hypotheses we cannot validate faster than we already kill them. Pricing verified 2026-09-10; re-verify before any call |
...
| Helius / Birdeye "top trader" or wallet-ranking endpoints | helius.dev, birdeye.so | **REJECTED 2026-09-13** as a copy-trading wallet universe. Any list ranked by realised performance is survivorship-selected; buying it from a vendor rather than reading a public leaderboard changes the wrapper, not the bias. Not a cost question — a budget does not reopen it. See `planning/decisions/2026-09-13-copy-trading-wallet-universe.md`. Narrow exception recorded there: an indexed source used purely for *enumeration* ("which wallets traded pool P before date T", no performance ordering) would satisfy the point-in-time rule — but no such access exists now and looking for one is out of scope |
